...
`employee_email_domains` / `employee_email_allowlist` keys into
`githooks.EmployeeEmailCheck{Domains, Allowlist}`; update that wiring to the
new `AllowedDomains` shape.

## What changed

- `internal/cli/config.go`: `Config.EmployeeEmailDomains` +
...
- `internal/cli/integration_test.go`: replaced the two employee-email
  integration tests pinned to the old deny-list/off-by-default semantics:
  - `TestScanPrivacy_InternalEmailWarnsWithoutStrict` now configures
    ...
    address at that domain doesn't warn while an address at another domain
...
go build ./...          → ok
go vet ./...            → ok
go test ./... -count=1  → ok (all packages pass, including both new/updated
                            employee-email integration tests)
```
...
- Used a temporary local `replace` directive only to verify the wiring
  ...
  committed `go.mod`/`go.sum` are unchanged from `main` (still pinned to
...
- **This change cannot land usefully on its own**: it needs `go/githooks` to
  ...
  this repo bumping the `go/githooks` require line in `go.mod`/`go.sum` to
  that release, before `go build`/`go test` will pass without a local
